...
class Lie():
    ...
    def se3_to_SE3(self,wu): # [...,3]
        ...
        A = self.taylor_A(theta, nth=8)
        B = self.taylor_B(theta, nth=8)
        C = self.taylor_C(theta, nth=8)
...
def img2cam(X,intr_inv):
    return X@intr_inv.transpose(-1,-2)
...
def get_center_and_ray(opt,pose,intr_inv=None): # [HW,2]
    ...
    grid_3D = img2cam(to_hom(xy_grid),intr_inv) # [B,HW,3]
    ...
    ray_dirs = grid_3D @ R_inv
    centers = -(t.transpose(-2,-1) @ R_inv).repeat(1, opt.H*opt.W, 1)
...
def get_camera_cords_grid_3D(opt,batch_size,intr_inv=None,ray_idx=None): # [HW,2]
...
def gather_camera_cords_grid_3D(opt,batch_size,intr_inv=None,ray_idx=None): # [HW,2]
    # given the intrinsic matrices, get the grid_3D]
    assert(opt.camera.model=="perspective")
    with torch.no_grad():
        # compute image coordinate grid
...
def convert_NDC(opt,center,ray,intr,near=0.1):
        ...
        if hasattr(opt.arch, "ndc_near_plane"):
            near = opt.arch.ndc_near_plane
        if hasattr(opt.arch ,"ndc_center_shift") and opt.arch.ndc_center_shift == False:
            pass
        else:
            if hasattr(opt.arch, "detach_ndc_center_shift") and opt.arch.detach_ndc_center_shift:
                center = center+((near-center[...,2:])/ray[...,2:]*ray).detach()
            else:
        ...
        cxoz, cyoz = cx/cz, cy/cz
        rxoz, ryoz = rx/rz, ry/rz
        # cnx = scale_x[:,None]*(cx/cz)
        # cny = scale_y[:,None]*(cy/cz)
        cnx = scale_x[:,None]*(cxoz)
        cny = scale_y[:,None]*(cyoz)
        cnz = 1-2*near/cz

        # rnx = scale_x[:,None]*(rx/rz-cx/cz)
        # rny = scale_y[:,None]*(ry/rz-cy/cz)
        rnx = scale_x[:,None]*(rxoz-cxoz)
        rny = scale_y[:,None]*(ryoz-cyoz)
        rnz = 2*near/cz
...
def get_novel_view_around_bbox(opt, N=120, scale=1):
    # create surrounding view point around bbox
    bbox = torch.tensor(opt.data.scene_bbox).view(2,3).to(opt.device)
    center = torch.mean(bbox, dim=0)
    norm = torch.norm(bbox[0]-bbox[1])
    radius = norm * 0.2 * scale
    theta = torch.arange(N)/N*2*np.pi
    theta = theta.to(opt.device)
    camera_dist = norm * 0.6 * scale

    bbox_height = torch.abs(bbox[1,1] - bbox[0,1])
    h_above_center = bbox_height * 0.1 * scale
    pitch_angle = torch.atan(h_above_center / camera_dist )
    R_x = angle_to_rotation_matrix(pitch_angle ,"X")
    R_y = angle_to_rotation_matrix(theta, "Y")
    R_dwn_90 = angle_to_rotation_matrix(torch.full_like(theta, np.pi/2), "X")

    pose_x = torch.zeros_like(theta)
    pose_z = torch.full_like(theta, camera_dist)
    pose_y = torch.full_like(theta, h_above_center)

    novel_pose = pose(R=R_x@R_y@R_dwn_90, t=torch.stack([pose_x, pose_y, pose_z], dim=-1))
    return novel_pose
