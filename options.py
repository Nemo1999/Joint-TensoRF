...
def parse_arguments(args):
    ...
    for arg in args:
        ...
        for k in keys_sub[:-1]:
            ...
            opt_sub = opt_sub[k]
        assert (keys_sub[-1] not in opt_sub), f"duplicate assign for key {keys_sub.join('.')}"
        # convert scientific string into float, because there is a bug in the yaml parser, see issue https://github.com/yaml/pyyaml/issues/173
...
def process_options(opt):
    ...
    opt.H,opt.W = map(int,opt.data.image_size)
