...
def restore_checkpoint(opt,model,load_name=None,resume=False):
    ...
    # manually load non state_dict paramters of model
    if hasattr(model, "load_param_state"):
        param_state = checkpoint["manually_tracked_parameters"]
        param_state.update({"iter":checkpoint["iter"]})
        model.load_param_state(opt, param_state)
    # load individual (possibly partial) children modules

    # remove saved se3 weight if the train_data is different
    if "se3_refine.weight" in checkpoint["graph"] and checkpoint["graph"]["se3_refine.weight"].shape[0] != len(model.train_data):
        del checkpoint["graph"]["se3_refine.weight"]
    for name,child in model.graph.named_children():
        ...
        if child_state_dict:
            ...
            child.load_state_dict(child_state_dict, strict=False)

    # load parameters in graph(parameters)

    # new se3 paramters may be different from


    model.graph.load_state_dict(checkpoint["graph"], strict=False)
...
def save_checkpoint(opt,model,ep,it,latest=False,children=None):
    ...
    # manually save non state_dict paramters of model
    if hasattr(model, "save_param_state"):
        param_state = model.save_param_state()
        checkpoint.update({"manually_tracked_parameters": param_state})
...
def grad_rescale(source, target):
    # rescale source tensor to target magnitude in backward path ( but does't affect foward path )
    weight = torch.abs(target) / torch.clamp(torch.abs(source),min=0.1, max=None)
    scaled_source = source * weight
    return  scaled_source + (source - scaled_source).detach()
