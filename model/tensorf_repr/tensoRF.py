...
class TensorVM(TensorBase):
    ...
    def init_svd_volume(self, res, device):
        self.plane_coef = torch.nn.Parameter(
            torch.abs(0.1 * torch.randn((3, self.app_n_comp + self.density_n_comp, res, res), device=device, dtype=self.dtype)))
        self.line_coef = torch.nn.Parameter(
            torch.abs(0.1 * torch.randn((3, self.app_n_comp + self.density_n_comp, res, 1), device=device, dtype=self.dtype)))
    ...
    def compute_features(self, xyz_sampled):
        ...
        if self.component_wise_feature2density:
            planes = self.feature2density(self.plane_coef[:, -self.density_n_comp:])
            lines = self.feature2density(self.line_coef[:, -self.density_n_comp:])
        else:
            planes = self.plane_coef[:, -self.density_n_comp:]
            liness = self.line_coef[:, -self.density_n_comp:]
        plane_feats = F.grid_sample(planes, coordinate_plane, align_corners=True).view(
                                        -1, *xyz_sampled.shape[:1])
        line_feats = F.grid_sample(lines, coordinate_line, align_corners=True).view(
                                        -1, *xyz_sampled.shape[:1])
...
class TensorVMSplit(TensorBase):
    ...
    def init_svd_volume(self, res, device,init_density=True, init_app=True, init_basis=True, init_scale=0.1, init_bias=0.1):
        if init_density:
            self.density_plane, self.density_line = self.init_one_svd(self.density_n_comp, self.gridSize, init_scale, init_bias,  device)
        if init_app:
            self.app_plane, self.app_line = self.init_one_svd(self.app_n_comp, self.gridSize, init_scale, init_bias, device)
        if init_basis:
            self.basis_mat = torch.nn.Linear(sum(self.app_n_comp), self.app_dim, bias=False).to(device, dtype=self.dtype)


    def init_one_svd(self, n_component, gridSize, scale, bias, device):
        plane_coef, line_coef = [], []
        for i in range(len(self.vecMode)):
            vec_id = self.vecMode[i]
            mat_id_0, mat_id_1 = self.matMode[i]
            plane_coef.append(torch.nn.Parameter(
                torch.abs(bias + scale * torch.randn((1, n_component[i], gridSize[mat_id_1], gridSize[mat_id_0]), dtype=self.dtype))))  # 1, n_comp, resolution0, resolution1
            line_coef.append(
                torch.nn.Parameter(torch.abs(bias + scale * torch.randn((1, n_component[i], gridSize[vec_id], 1), dtype=self.dtype)))) # 1, n_comp, resolution2

        return torch.nn.ParameterList(plane_coef).to(device), torch.nn.ParameterList(line_coef).to(device)
    def freeze_scene(self, opt):
        self.basis_mat.weight.requires_grad = False
        self.renderModule.requires_grad_(False)
        for i in range(3):
            self.density_plane[i].requires_grad = False
            self.density_line[i].requires_grad = False
            self.app_plane[i].requires_grad = False
            self.app_line[i].requires_grad = False


    def unfreeze_scene(self, opt):
        self.basis_mat.weight.requires_grad = True
        self.renderModule.requires_grad_(True)
        for i in range(3):
            self.density_plane[i].requires_grad = True
            self.density_line[i].requires_grad = True
            self.app_plane[i].requires_grad = True
            self.app_line[i].requires_grad = True
    ...
    def compute_densityfeature(self, xyz_sampled):
        ...
        for idx_plane in range(len(self.density_plane)):
            ...
            plane_coef_point = F.grid_sample(planes, coordinate_plane[[idx_plane]],
                                                align_corners=True).view(-1, *xyz_sampled.shape[:1])
            line_coef_point = F.grid_sample(lines, coordinate_line[[idx_plane]],
                                            align_corners=True).view(-1, *xyz_sampled.shape[:1])
...
class TensorCP(TensorBase):
    ...
    def init_svd_volume(self, res, device,init_density=True, init_app=True, init_basis=True, init_scale=0.1):
        if init_density:
             self.density_line = self.init_one_svd(self.density_n_comp, self.gridSize, init_scale, device)
        if init_app:
             self.app_line = self.init_one_svd(self.app_n_comp, self.gridSize, init_scale, device)
        if init_basis:
            self.basis_mat = torch.nn.Linear(self.app_n_comp[0], self.app_dim, bias=False).to(device, dtype=self.dtype)
    ...
    def init_one_svd(self, n_component, gridSize, scale, device):
        ...
        for i in range(len(self.vecMode)):
            vec_id = self.vecMode[i]
            line_coef.append(
                torch.nn.Parameter(scale * torch.randn((1, n_component[i], gridSize[vec_id], 1),dtype=self.dtype)))
    ...
    def compute_densityfeature(self, xyz_sampled): # B, n_sample, 3
        ...
        coordinate_line = torch.stack((torch.zeros_like(coordinate_line), coordinate_line), dim=-1).detach().view(3, -1, 1, 2) # 3, B*n_sample, 1, 2 (grid_sample grid)
        #ic(xyz_sampled)
        ic(xyz_sampled.shape)

        #ic(coordinate_line)
        ic(coordinate_line.shape)
        # torch.nn.grid_sample in 4D case
        # input with shape (N,C,H_in,W_in)
        # grid with shape (N, H_out, W_out, 2)
        # output with shape (N, C, H_out, W_out)
        line_coef_point = F.grid_sample(self.density_line[0], coordinate_line[[0]], # 1, self.density_n_component[i] , B*n_sample, 1
    ...
    def compute_appfeature(self, xyz_sampled):# B, n_sample, 3
...
"""
class TensorVM_MultiScale(TensorVMSplit):
    def __init__(self, aabb, gridSize, device, initGridSize,**kargs):
        super(TensorVM_MultiScale, self).__init__(aabb, gridSize, device, **kargs)

    def init_svd_volume(self, res, device, init_density=True, init_app=True, init_basis=True, init_scale=0.1):
        self.density_planes[], self.density_lines[], self.app_planes[]
        for scale
        if init_density:
            self.density_plane, self.density_line = self.init_one_svd(self.density_n_comp, self.gridSize, init_scale, device)
        if init_app:
            self.app_plane, self.app_line = self.init_one_svd(self.app_n_comp, self.gridSize, init_scale, device)
        if init_basis:
            self.basis_mat = torch.nn.Linear(sum(self.app_n_comp), self.app_dim, bias=False).to(device, dtype=self.dtype)
"""
