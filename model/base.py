...
class Model(torch.nn.Module):
    ...
    def load_dataset(self,opt,eval_split="val", train_split="train"):
        ...
        self.train_data = data.Dataset(opt,split=train_split,subset=opt.data.train_sub)
        self.train_loader = self.train_data.setup_loader(opt,shuffle=False)
        log.info("loading test data...")
        if opt.data.val_on_test: eval_split = "test"
        subset = opt.data.test_sub if eval_split == "test" else opt.data.val_sub
        self.test_data = data.Dataset(opt,split=eval_split,subset=subset)
    ...
    def train_iteration(self,opt,var,loader):
        ...
        if hasattr(opt,"profiling") and opt.profiling == True:
            ...
            # train iteration
            with record_function("graph.forward"):
                var = self.graph.forward(opt,var,mode="train")
            with record_function("graph.compute_loss"):
                loss = self.graph.compute_loss(opt, var, mode="train")
                ic("after graph.compute_loss")
                ic(self.graph.nerf.tensorf.density_plane[0].grad)
                ic(max(self.graph.nerf.tensorf.density_plane[0]))
            with record_function("summarize_loss"):
                loss = self.summarize_loss(opt,var,loss)
                ic("after summarize_loss")
                ic(self.graph.nerf.tensorf.density_plane[0].grad)
                ic(max(self.graph.nerf.tensorf.density_plane[0]))
            with record_function("loss.all.backward()"):
                loss.all.backward()
                ic("after loss.all.backward")
                ic(torch.max(self.graph.nerf.tensorf.density_plane[0].grad))
                ic(max(self.graph.nerf.tensorf.density_plane[0]))
            with record_function("optim.step"):
                self.optim.step()
                ic("after optim.step")
                ic(torch.max(self.graph.nerf.tensorf.density_plane[0].grad))
                ic(self.graph.nerf.tensorf.density_plane[0])
    ...
    def validate(self,opt,ep=None):
        ...
        for it,batch in enumerate(loader):
            var = edict(batch)
            # we have to modify self.it to make validation work (when we call Model without training, it is not set)
            self.it = it
            var = util.move_to_device(var,opt.device)
...
class Graph(torch.nn.Module):
    ...
    def MSE_loss(self,pred,label=0):
        ...
        return loss.nanmean() # sometimes rendered image contains nan, temporary solution to ignore the nan pixels
